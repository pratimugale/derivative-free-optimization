...
class SimplexOriginal(Optimizer):
    ...
    def update_points(self, x_cap: Point, points: np.ndarray):
        ...
        if(F_x_cap < points[self.num_parameters - 1].val_f): # compare with the (n-1)-th variable's function value
            print(f'n-1 th point is {points[self.num_parameters - 1].val_f} and thus {F_x_cap} is smaller than it')
            # points.pop(-1)
            ...
            print(f'Contration needs to be performed! n-1 th point is {points[self.num_parameters - 1].val_f} and thus {F_x_cap} is greater than or equal to it')
